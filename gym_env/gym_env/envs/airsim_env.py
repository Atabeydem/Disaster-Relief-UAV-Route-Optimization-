            ...
            if self.model is not None:
                with th.no_grad():
                    ...
                    q_value_current = self.model.critic(th.from_numpy(obs_copy[tuple(
                        [None])]).float().cpu(), th.from_numpy(action[None]).float().cpu())
...
# ! -------------------------get obs------------------------------------------
    ...
    def get_orig_depth_image(self):
        """image = Image.fromarray(self.get_orig_rgb_image())
        ...
        image_array = (output * 255 / np.max(output)).astype("uint8")"""
        ...
        return cm.inferno(np.array(image_array))

    def get_orig_rgb_image(self):
        responses = self.client.simGetImages(
        ...
        image_array = Image.fromarray(image[:, :, ::-1]).resize((84, 84)).convert("RGB")
...
# ! ---------------------calculate rewards-------------------------------------
    ...
    def compute_reward_final(self, done, action):
        ...
        if not done:
            ...
            punishment_z = np.clip((z - z_g)/5, 0, 1)
...
# ! -----------used for plot or show states------------------------------------------------------------------
    ...
    def set_pyqt_signal_fixedwing(self, action, reward, done):
        ...
        self.rgb_signal.emit(np.asarray(self.get_orig_rgb_image()))
        self.depth_signal.emit(np.asarray(self.get_orig_depth_image()))
