...
class TrainingUi(QWidget):
    ...
    def init_ui(self):
        ...
        #palette.setColor(QPalette.Window, QColor(0, 183, 235))
        #self.setPalette(palette)
        self.setStyleSheet("QWidget {background-color: qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #ADD8E6, stop: 1 #00008B)}")
        ...
        rgb_plot_gb = self.create_rgb_plot_groupbox()
        depth_plot_gb = self.create_depth_plot_groupbox()
        ...
        left_widget = QWidget()
        leftcol_layout = QVBoxLayout()
        leftcol_layout.addWidget(state_plot_gb)

        leftdown_widget = QWidget()
        leftdown_row = QHBoxLayout()
        leftdown_row.addWidget(rgb_plot_gb)
        leftdown_row.addWidget(depth_plot_gb)
        leftdown_widget.setLayout(leftdown_row)
        leftcol_layout.addWidget(leftdown_widget)

        left_widget.setLayout(leftcol_layout)
...
# action plot groupbox
    ...
    def create_state_plot_groupbox(self):
        state_plot_groupbox = QGroupBox(title='State')
        ...
        self.pixmap = QPixmap('HAVASUR.png')
        self.label.setPixmap(self.pixmap)
        custom_width = 50  
        custom_height = 50  
        self.label.resize(custom_width, custom_height)
        self.label.setAlignment(Qt.AlignTop | Qt.AlignLeft)"""
        logo_pw = pg.PlotWidget()
        img_data = Image.open("resources/HAVASUR.png")
        image = np.copy(img_data)
        logo = pg.ImageItem(image)
        logo_pw.showGrid(x=False, y=False)
        logo_pw.invertY()
        logo_pw.addItem(logo)
        logo_pw.getPlotItem().getAxis('bottom').setStyle(showValues=False)
        logo_pw.getPlotItem().getAxis('left').setStyle(showValues=False)
        layout.addWidget(logo_pw)
...
# trajectory plot groupbox
    ...
    def create_rgb_plot_groupbox(self):
        traj_plot_groupbox = QGroupBox('Drone Camera (RGB)')
        traj_plot_groupbox.setFixedHeight(600)
        traj_plot_groupbox.setFixedWidth(600)
        layout = QVBoxLayout()

        self.rgb_pw = pg.PlotWidget(title='Drone Camera (RGB)')
        #self.rgb_pw.showGrid(x=True, y=True)
        # self.traj_pw.setXRange(max=350, min=-100)
        # self.traj_pw.setYRange(max=100, min=-300)
        #self.rgb_pw.setXRange(max=140, min=-140)
        #self.rgb_pw.setYRange(max=140, min=-140)
        self.rgb_pw.plot()
        self.rgb_pw.invertY()
        ...
        img_data = Image.open(background_image_path)
        image = np.copy(img_data)
        self.rgb_img = pg.ImageItem(image)"""
        if self.rgb_img is not None:
            self.rgb_pw.addItem(pg.ImageItem(self.rgb_img))
            """
            # make sure image is behind other data
            self.rgb_img.setZValue(-100)
            self.rgb_img.setRect(pg.QtCore.QRectF(-135, -135, 270, 270))"""
            #self.rgb_pw.setXRange(max=135, min=-135)
            #self.rgb_pw.setYRange(max=135, min=-135)
        
        layout.addWidget(self.rgb_pw)
        traj_plot_groupbox.setLayout(layout)
        return traj_plot_groupbox
    
    def create_depth_plot_groupbox(self):
        traj_plot_groupbox = QGroupBox('Generated Depth Image')
        traj_plot_groupbox.setFixedHeight(600)
        traj_plot_groupbox.setFixedWidth(600)
        layout = QVBoxLayout()

        self.depth_pw = pg.PlotWidget(title='Generated Depth Image')
        #self.depth_pw.showGrid(x=True, y=True)
        # self.traj_pw.setXRange(max=350, min=-100)
        # self.traj_pw.setYRange(max=100, min=-300)
        #self.depth_pw.setXRange(max=140, min=-140)
        #self.depth_pw.setYRange(max=140, min=-140)
        self.depth_pw.plot()
        self.depth_pw.invertY()

        """ background_image_path = 'resources/env_maps/NH_center.png'
        img_data = Image.open(background_image_path)
        image = np.copy(img_data)
        self.depth_img = pg.ImageItem(image)"""
        if self.depth_img is not None:
            self.depth_pw.addItem(pg.ImageItem(self.depth_img))
            """
            # make sure image is behind other data
            self.depth_img.setZValue(-100)
            self.depth_img.setRect(pg.QtCore.QRectF(-135, -135, 270, 270))"""
            #self.depth_pw.setXRange(max=135, min=-135)
            #self.depth_pw.setYRange(max=135, min=-135)

        layout.addWidget(self.depth_pw)
        traj_plot_groupbox.setLayout(layout)
        return traj_plot_groupbox
    ...
    def traj_plot_cb(self, goal, start, current_pose, trajectory_list):
        ...
        self.traj_pw.plot([start[0]], [start[1]], symbol='o', symbolSize=15)
        self.traj_pw.plot([goal[0]], [goal[1]], symbol='o', symbolSize=15)
        self.traj_pw.plot(
            trajectory_list[..., 0], trajectory_list[..., 1], pen={'color': 'r', 'width': 5})
